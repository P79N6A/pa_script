...
def loads(string, *args):
    ...
    # try:
    #     opcode, res = marshaller._read_and_exec_opcode(ident=0)

    # res, left = marshaller.readObject()
    # while(left){
    #     res, left = marshaller.readObject()
    # }
    return marshaller.readObject()
...
#                    field_type = "array of None"
    ...
    def _create_hexdump(self, src, length=16):
        ...
        for i in range(0, len(src), length):
            s = src[i:i+length]
            hexa = ' '.join(["%02X"%ord(chr(x)) for x in s])
            printable = s.translate(FILTER.encode())
