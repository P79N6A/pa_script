...
from PA.InfraLib.ModelsV2.IM.QQ import *
import os
import sqlite3
import time
...
SQL_INSERT_TABLE_CHATROOM_MEMBER = '''
    insert into chatroom_member(account_id, chatroom_id, member_id,nick_name, display_name, sp_id, source, deleted, repeated)
        values(?, ?, ?, ?, ?, ?, ?, ?, ?)'''
    ...
    def db_create(self, db_path):
        if os.path.exists(db_path):
            try:
                ...
                TraceService.Trace(TraceLevel.Error, "model_qq.py Error: LINE {}".format(traceback.format_exc()))
                return False
...
class ChatroomMember(Column):
    ...
    def get_values(self):
        return (self.account_id, self.chatroom_id, self.member_id, self.nick_name,self.display_name, self.sp_id) + super(ChatroomMember, self).get_values()
...
class GenerateModel(object):
    ...
    def get_models(self):
        ...
        self.set_progress(5)
        #print('%s model_qq() generate model friend' % time.asctime(time.localtime(time.time())))
        self._get_friend_models()
        self.set_progress(15)
        #print('%s model_qq() generate model group' % time.asctime(time.localtime(time.time())))
        self._get_group_models()
        self.set_progress(25)
        #print('%s model_qq() generate model contact label' % time.asctime(time.localtime(time.time())))
        self._get_contact_label_models()
        self.set_progress(26)
        #print('%s model_qq() generate model feed' % time.asctime(time.localtime(time.time())))
        self._get_feed_models()
        self.set_progress(45)
        #print('%s model_qq() generate model message' % time.asctime(time.localtime(time.time())))
        self._get_message_models()
        self.set_progress(85)
        #print('%s model_qq() generate model search' % time.asctime(time.localtime(time.time())))
        self._get_search_models()
        self.set_progress(90)
        #print('%s model_qq() generate model favorite' % time.asctime(time.localtime(time.time())))
        self._get_favorite_models()
        self.set_progress(100)
        #print('%s model_qq() generate model end' % time.asctime(time.localtime(time.time())))
        ...
        try:
            ...
            while r.Read():
                ...
                try:
                    ...
                    if deleted == 0 or self._gen_key(account_id, account_id) not in self.friend_models:
                        friend = QQ.Friend()
                        ...
                        friend.Type = QQ.FriendType.Friend
                        self.friend_models[self._gen_key(account_id, account_id)] = friend
                        self.add_model(friend)
                except Exception as e:
                    if deleted == 0:
                        TraceService.Trace(TraceLevel.Error, "model_qq.py Error: db:{} LINE {}".format(self.cache_db, traceback.format_exc()))
            self.push_models()
        except Exception as e:
            TraceService.Trace(TraceLevel.Error, "model_qq.py Error: db:{} LINE {}".format(self.cache_db, traceback.format_exc()))
        ...
        try:
            ...
            while r.Read():
                ...
                try:
                    ...
                    if deleted == 0 or self._gen_key(account_id, user_id) not in self.friend_models:
                        self.friend_models[self._gen_key(account_id, user_id)] = model
                except Exception as e:
                    if deleted == 0:
                        TraceService.Trace(TraceLevel.Error, "model_qq.py Error: db:{} LINE {}".format(self.cache_db, traceback.format_exc()))
            self.push_models()
        ...
        try:
            ...
            while r.Read():
                ...
                try:
                    ...
                    account_id = self._db_reader_get_string_value(r, 0)
                    chatroom_id = self._db_reader_get_string_value(r, 1)
                    nickname = self._db_reader_get_string_value(r, 2)
                    ...
                    model.Deleted = self._convert_deleted_status(deleted)
                    model.AppUserAccount = self.account_models.get(account_id)
                    model.Account = chatroom_id
                    model.NickName = nickname
                    model.HeadPortraitPath = photo
                    model.Notice = notice                    
                    model.Members.AddRange(self._get_chatroom_member_models(account_id, chatroom_id, sp_id, deleted))
                    model.JoinTime = self._get_timestamp(join_time)
                    self.add_model(model)

                    if deleted == 0 or self._gen_key(account_id, chatroom_id) not in self.chatroom_models:
                        self.chatroom_models[self._gen_key(account_id, chatroom_id)] = model
                except Exception as e:
                    if deleted == 0:
                        TraceService.Trace(TraceLevel.Error, "model_qq.py Error: db:{} LINE {}".format(self.cache_db, traceback.format_exc()))
            self.push_models()
            ...
            while r.Read():
                ...
                try:
                    ...
                    if msg_type == MESSAGE_CONTENT_TYPE_IMAGE:
                        ...
                        model.Content.Value = QQ.BusinessCard()
                        ...
                        model.Content.Value = QQ.RedEnvelope()
                        ...
                        model.Content.Value = QQ.SplitBill()
                        model.Content.Value.Expiration = self._get_timestamp(deal_expire_time)
                        model.Content.Value.Title = deal_description
                        model.Content.Value.Remark = deal_remark
                        ...
                        model.Content.Value = QQ.Transfer()
                        model.Content.Value.Expiration = self._get_timestamp(deal_expire_time)
                        model.Content.Value.Title = deal_description
                        model.Content.Value.Remark = deal_remark
            ...
            while r.Read():
                ...
                try:
                    ...
                    if member_id not in [None, '']:   
                        ...
                        user.SourceFile = source
                        deleted = self._db_reader_get_int_value(r, 3)
                        user.Deleted = self._convert_deleted_status(deleted)
                        user.AppUserAccount = self.account_models.get(account_id)
                        ...
                        groupMember.User = user
                        self.group_member_models[self._gen_key(chatroom_id, member_id)] = groupMember
                        model = groupMember
                        if model is not None:
                            models.append(model)
                except Exception as e:
                ...
                try:
                    ...
                    if cl_type == CONTACT_LABEL_TYPE_GROUP:
                        ...
                        for user_id in users:
                            friend = self.friend_models.get(self._gen_key(account_id, user_id))
    ...
    def _gen_key(self, account_id, user_id):
        ...
        try:
            ...
            while r.Read():
                ...
                try:
                    ...
                    model.SourceFile = source
                    model.Deleted = self._convert_deleted_status(deleted)
                    model.Sender = self.friend_models.get(self._gen_key(feed_model.AppUserAccount.Account, sender_id))
                    model.CreateTime = self._get_timestamp(timestamp)
                    models.append(model)
                except Exception as e:
                    if deleted == 0:
        ...
        try:
            ...
            while r.Read():
                ...
                try:
                    ...
                    model.SourceFile = source
                    model.Deleted = self._convert_deleted_status(deleted)
                    model.From = self.friend_models.get(self._gen_key(feed_model.AppUserAccount.Account, sender_id))
                    if ref_user_id not in [None, '']:
                        model.To = self.friend_models.get(self._gen_key(feed_model.AppUserAccount.Account, ref_user_id))
    ...
    def _convert_friend_type(friend_type):
        if friend_type == FRIEND_TYPE_FRIEND:
            return QQ.FriendType.Friend
        elif friend_type == FRIEND_TYPE_OFFICIAL:
            return QQ.FriendType.Official
        else:
            return QQ.FriendType.None
