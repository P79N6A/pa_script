...
class TaobaoParser(object):
    def __init__(self, root, extract_deleted, extract_source):
        ...
        self.model_eb_col, self.model_im_col = self.__load_eb_models(self.cache_db, Taobao_VERSION, self.app_name)
    ...
    def __get_cache_db(self):
        """获取中间数据库的db路径"""
        self.cache_path = ds.OpenCachePath("Taobao")
        m = hashlib.md5()
    ...
    def __load_eb_col(self, db_name):
    ...
    def __generate_accounts(self):
        files = Utils.list_dir(self.data_path)
        for f in files:
            if f.startswith("bizDB_") and not (f.endswith("-shm") or f.endswith("-wal")):
                account_username = f[6:-10]
                account_id = f[-10:]
                yield account_id, account_username
    ...
    def _generate_account_table(self, account_username, account_id):
        with self.checking_account_col as db_col:
            sql = """SELECT PROFILE_ID, 
                            IDENTITY_TYPE, 
                            DISPLAY_NAME, 
    ...
    def __fetch_sender_name(self, sender_id):
        sql = """SELECT NICK FROM profile where PROFILE_ID = '{}'""".format(sender_id)
    ...
    def _generate_eb_data(self):
        ...
        self.eb_shop_col = self.__load_eb_col("AmpData")
        self.eb_products_col = self.__load_eb_col("data_history")
        self.eb_log_col = self.__load_eb_col("MLTK.db")
...
def parse_Taobao(root, extract_deleted, extract_source):
    pr = ParserResults()
    pr.Categories = DescripCategories.Taobao
    results = TaobaoParser(root, extract_deleted, extract_source).parse()
    if results:
